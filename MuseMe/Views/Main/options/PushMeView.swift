    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Image("finallogo")
                ...
                .opacity(session.isSessionActive && session.currentMode == .push ? 1.0 : 0.3)
                .animation(.easeInOut(duration: 0.4), value: session.isSessionActive)

            Text(session.isSessionActive && session.currentMode == .push ? timerString : "00:00")
                .font(.system(size: 48, weight: .thin, design: .monospaced))
                .foregroundColor(session.isSessionActive && session.currentMode == .push ? .white : Color.gray.opacity(0.4))
                .padding(.top, 28)
